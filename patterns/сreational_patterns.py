...
class CourseMapper:
    ...
    def find_by_id(self, id):
        statement = f"SELECT id, course_name FROM {self.tablename} WHERE id=?"
        self.cursor.execute(statement, (id,))
        result = self.cursor.fetchone()
        if result:
            return Category(*result)
    ...
    def insert(self, obj):
        statement = f"INSERT INTO {self.tablename} (course_name) VALUES (?)"
        self.cursor.execute(statement, (obj.name,))
        try:
            self.connection.commit()
        except Exception as e:
    ...
    def update(self, obj):
        statement = f"UPDATE {self.tablename} SET course_name=? WHERE id=?"
